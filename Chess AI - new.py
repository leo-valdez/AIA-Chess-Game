...
Elargefont = pygame.font.Font("GOUDOS.ttf", 120)
Mlargefont = pygame.font.Font("calibri.ttf", 80)
board=chess.Board()
def drawBoard():
    global board
...
def negamax(val,depth):
        ...
        if len(l) is not 0:
            ...
            for i in l:
                ...
                if len(l1) is 0:
                    board.pop()
                    return [i,eval_board(i)]
                else:
                    k1=[]
                    for j in l1:
                        board.push(j)
                        k1.append([j,eval_board(j)])
                        board.pop()
                    p=generate_minmax(k1,(-1*val))
                    k.append([i,k1[p]])
                board.pop()
            if val is 1:
                i=[]
